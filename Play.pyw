...
root = tk.Tk()
root.title("Face to Bomb")
root.geometry("300x200")  # initial size

face_label = tk.Label(root, text="", font=("Helvetica", 48))
...
def move_window(times_left):
    if times_left > 0:
        ...
        root.geometry(f"+{x}+{y}")
        root.after(200, lambda: move_window(times_left - 1))
    else:
        root.destroy()
...
def fake_delete_system32(progress=0):
    if progress <= 100:
        ...
        root.geometry("")  # auto-resize window to fit contents
        root.after(100, lambda: fake_delete_system32(progress + 5))
    else:
        face_label.config(text="System32 successfully deleted! 💀")
        timer_label.config(text="")
        root.update_idletasks()
        root.geometry("")
        root.after(1000, lambda: move_window(20))
...
def update_bomb(time_left):
    if time_left > 0:
        ...
        root.update_idletasks()
        root.geometry("")
        root.after(1000, lambda: update_bomb(time_left - 1))
    else:
        face_label.config(text="💥 BOOM!")
        timer_label.config(text="")
        root.update_idletasks()
        root.geometry("")
        root.after(1000, fake_delete_system32)
